...
lisbon_road_network_esri = sf::read_sf("https://github.com/ropensci/slopes/releases/download/0.0.0/route_segments_gradient2_Zelevation.gpkg")
...
lisbon_road_segment = lisbon_road_network |>
  dplyr::filter(OBJECTID == 2997)
usethis::use_data(lisbon_road_segment, overwrite = TRUE)
lisbon_road_segment_3d = download.file("https://github.com/geocompr/d/releases/download/1/r1.zip", "r1.zip")
unzip("r1.zip")
dem_lisbon_raster = terra::rast("r1")
dem_lisbon_raster = terra::crop(dem_lisbon_raster, lisbon_1km_projected)
dem_lisbon_raster = terra::trim(dem_lisbon_raster)
terra::crs(dem_lisbon_raster) # no CRS
terra::plot(dem_lisbon_raster)
...
terra::writeRaster(dem_lisbon_raster, "inst/dem_lisbon_raster.asc", overwrite = TRUE)
file.size("inst/dem_lisbon_raster.asc") / 1e6 # large, so deleted

# With terra
lisbon_road_network_terra = terra::vect("https://github.com/ropensci/slopes/releases/download/0.0.0/route_segments_gradient2_Zelevation.gpkg")
